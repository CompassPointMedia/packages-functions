<?php
$functionVersions['stats_collection']=1.10;
function stats_collection($secure=false, $options=array()){
	global $fromHdrBugs, $developerEmail, $enhanced_parse_url, $MASTER_DATABASE, $qx, $qr, $fl, $ln;
	/**
	...
	**/
	...
	if($enhanced_parse_url['scheme'] && $enhanced_parse_url['domain'] && $enhanced_parse_url['rawpath'] && $enhanced_parse_url['file']){
		...
		if(!($Schemes_ID=q("SELECT ID FROM stats_protocols WHERE Name='".strtolower($a['scheme'])."'",O_VALUE,ERR_ECHO, $cnx))){
			$Schemes_ID=q("INSERT INTO stats_protocols SET Name='".strtolower($a['scheme'])."'",O_INSERTID,ERR_ECHO, $cnx);
		}
		//remember Name is binary
		if(!($Domains_ID=q("SELECT ID FROM stats_domains WHERE Name='".strtolower($a['domain'])."'",O_VALUE,ERR_ECHO, $cnx))){
			$Domains_ID=q("INSERT INTO stats_domains SET Name='".strtolower($a['domain'])."'",O_INSERTID,ERR_ECHO, $cnx);
		}
		if(!($Paths_ID=q("SELECT ID FROM stats_paths WHERE Name='".$a['rawpath']."'",O_VALUE,ERR_ECHO, $cnx))){
			$Paths_ID=q("INSERT INTO stats_paths SET Name='".$a['rawpath']."'",O_INSERTID,ERR_ECHO, $cnx);
		}
		...
		if($maxViewTime=q("SELECT DATE_FORMAT(MAX(ViewTime),'%Y-%m-%d %H:%i:%s') FROM stats_VisitorsPageserved WHERE Ips_ID='$Ips_ID'", O_VALUE,ERR_ECHO, $cnx)
			&&
		...
		**/
		...
		if($statsUserID){
			...
			**/
			if(!($Visitors_ID=q("SELECT ID FROM stats_visitors WHERE Ips_ID='$Ips_ID' AND Machines_ID".($Machines_ID?"='$Machines_ID'":" IS NULL"),O_VALUE,ERR_ECHO, $cnx))){
		...
		if($_SERVER['HTTP_REFERER']){
			...
			if(!($Schemes_ID=q("SELECT ID FROM stats_protocols WHERE Name='".strtolower($b['scheme'])."'",O_VALUE,ERR_ECHO, $cnx))){
				$Schemes_ID=q("INSERT INTO stats_protocols SET Name='".strtolower($b['scheme'])."'",O_INSERTID,ERR_ECHO, $cnx);
			}
			...
			if(!($Paths_ID=q("SELECT ID FROM stats_paths WHERE Name='".$b['rawpath']."'",O_VALUE,ERR_ECHO, $cnx))){
				$Paths_ID=q("INSERT INTO stats_paths SET Name='".$b['rawpath']."'",O_INSERTID,ERR_ECHO, $cnx);
			}
		...
		if($Visits_ID=q("SELECT ID FROM stats_visits WHERE Visitors_ID=$Visitors_ID AND LastViewTime + INTERVAL 2 HOUR > NOW()", O_VALUE,ERR_ECHO, $cnx)){
			q("UPDATE stats_visits SET PagesViewed=PagesViewed+1, LastViewTime=NOW(), Version='1.1' WHERE ID=$Visits_ID", ERR_ECHO, $cnx);
		}else{
			$Visits_ID=q("INSERT INTO stats_visits SET
			Visitors_ID=$Visitors_ID,
			PagesViewed=1,
			FirstViewTime=NOW(),
			Version='1.1'", O_INSERTID);
		}
