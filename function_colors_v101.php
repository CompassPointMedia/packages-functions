<?php
/************
2005-04-29: moved this as is over to dev.relatebase, this page is sunsetted and all further development
2005-03-07:
Improved color_read some, need to be able to select the output type
I now nearly have the ability to take a 3-color point plane.  What I really have is a "V" where the 4th color is intermediate between the arms of the v, so it's relatively in the plane.  I can pass this plane through saturation by drawing it into or out of the cone.  I can pass it through value by moving up or down.  I'm not sure about color, but presume I rotate each of the 3 (4) points around inside the cone without drawing in or moving up/down.
The main point ive seen is that the s and v movements should be linear, and at some poing we'll max out at 0 or 1.  Otherwise we have a plane that never really passes through the color cone at "radical" angles.
It would be good to somehow notate where we've hit those max boundaries.

todo list:
1. I have a few "holes" to fix in the color cubes - figure out why and correct.
2. Document the ideas
	a. a 3-color v in a single cube, vary some things while fixing others
	b. multiple cubes
split these pages off and have earth-level colors showing.  User wants to be able to specify ranges and do mixing.
3. I need to be able to click a color and another and have a comparison made on a sample page



************/
$functionVersions['color_read']=1.01;
function color_read($x,$mode='rgb',$output='array', $mode='startlike'){
	/************
	2005-03-07:
...
function color_mix($base, $overlay, $opacity=.5){
	$base=color_read($base);
	$overlay=color_read($overlay);
	$base=rgb2hsv($base[0],$base[1],$base[2]);
	$overlay=rgb2hsv($overlay[0],$overlay[1],$overlay[2]);
	$result[0]=round($base[0]+abs($overlay[0]-$base[0])*$opacity);
	$result[1]=round($base[1]+abs($overlay[1]-$base[1])*$opacity);
	$result[2]=round($base[2]+abs($overlay[2]-$base[2])*$opacity);
	$base=hsv2rgb($result[0],$result[1],$result[2]);
}
