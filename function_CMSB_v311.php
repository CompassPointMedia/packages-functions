...
function CMSB($section='', $method='', $options=array()){
	...
	*/
	...
	if(strtolower($method)=='static:default'){	
		...
		if(empty($CMSTable)) $CMSTable='cms1_articles';
		if(empty($CMSContentField)) $CMSContentField='Body';
		if(empty($primaryKeyField)) $primaryKeyField=array('ID');
		if(empty($primaryKeyFieldLabel)) $primaryKeyFieldLabel=array('Articles_ID');
...
function CMSBUpdate($options=array()){
	...
	*/
	...
	if(!empty($method) && $method==='static:default'){
		$Sections_ID=q("SELECT ID FROM cmsb_sections WHERE ThisFolder='".(isset($commonfolder) ? $commonfolder : (!empty($thisfolder) ? $thisfolder : ''))."' AND ThisPage='".(isset($commonpage) ? $commonpage : $thispage)."' AND Section='$thissection' AND Sections_ID IS NULL", O_VALUE, $cnx);
		prn($qr);
