...
$functionVersions['quickbooks_export_items']=4.01;
			...
			if(count($goShallow)){
				foreach($goShallow as $n=>$v){
					...
					//see if we need to track down parent items
															/*	'finan_accounts', 		external table
																'accnt',				external table alias
																'Name',					external table label field
																'Accounts_ID',			external table heirarchy field
																'ID',					external table pk
																'COGSAccounts_ID'		native table field
					LABEL_EXTERNAL_HIERARCHICAL
					...
					$body.=($fields[]=$rootTableAlias.'.'.($v[6]?$v[6]:$v[4]).' AS hierarchy_'.$n);
					$body.="\n";
	
	
					//orig.: $fields[]=($v[0]==LABEL_EXTERNAL_HIERARCHICAL ? $v[2].'.'.$v[4] : $rootTableAlias.'.'.$v[4]).' AS hierarchy_'.$n;
				}
			}
		...
		foreach($result as $rd){
			...
			foreach($rd as $n=>$v){
				...
				if($rd['hierarchy_'.$n]){ // && !isset($hierarchies[$n][$rd['hierarchy_'.$n]])
					...
					$str=$hierarchies[$n][$rd['hierarchy_'.$n]];
				}else{
					$str=trim( $convert[$n]? quickbooks_aux_convert($rd[$n], $convert[$n]) : $rd[$n] );
				}
		...
		while($value){
			$i++;
														/*	'finan_accounts',
															...
															'ID',
															'COGSAccounts_ID' */
			$sql='SELECT `'.$args[3].'`, `'.$args[4].'` FROM '.$args[1].' WHERE '.$args[5].'=\''.addslashes($value).'\'';
			$a=q($sql, O_ROW);
			$str=$a[$args[3]]. ($str?':':'') . $str;
			$value=$a[$args[4]];
			if($i==10){
				mail($developerEmail, 'Error in '.$MASTER_USERNAME.':'.end(explode('/',__FILE__)).', line '.__LINE__,get_globals($err='too many loops'),$fromHdrBugs);
