...
$functionVersions['t2']=1.02;
function t2(&$root,$param2='',$param3='',$param4='',$param5='',$param6='',$param7=''){
	/*
	2008-11-22 
	pass one parameter
		input=human, output then = unix [can do global setting to change]
		input=non-human, output = human, default format [can do global setting to change]
	two dates, 
		difference between them, an array of information
	we want to add time to a date

	basically input:read format => output:type => output:format

	already used as 2nd parameter
		o_datetime
		o_qbks
	*/
	if(!strlen($param1))return false;
	//configure args
	for($i=2;$i<=7;$i++){
		eval('if(strlen($param'.$i.'))$args[]=$param'.$i.';');
	}
	if(one date only){
		if($format=='error'){
			return false;
			...
			if(passed setting){
				return as passed setting
			}else if(override setting){
				return as override setting
			}
		}else{
			//we will be returning a human output
			if(passed setting){
				return as passed setting
			}else if(override setting){
				return as override setting
...
function t_parse($input){
	/*
	2008-11-22: Parse a date, then return a UNIX timestamp and the date's original format
	format values:
	-------------
	blank - zero string length
	datetime - mySQL
	datetime-blank - mySQL (for 0000-00-00 as normally returned)
	timestamp - mySQL; e.g. 20081122143015
	unix - integer
	human - anything besides the above
	error - unable to read; date portion is FALSE
