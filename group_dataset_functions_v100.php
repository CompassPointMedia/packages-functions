...
$functionVersions['dataset_function_parse']=1.00;
function dataset_function_parse($n){
	preg_match('/([^)]+\()([^)]+)/',$n,$m);
	if(preg_match('/^[-+a-z0-9%]+$/i',$m[2])){
		return $m[1].'"'.$m[2].'")';
	}else{
		$bs=md5(rand(1,1000000));
		$colon=md5(rand(1,1000000));
		$amp=md5(rand(1,1000000));
		$m[2]=str_replace('\\\\',$bs,$m[2]);
		$m[2]=str_replace('\:',$colon,$m[2]);
		$m[2]=str_replace('\&',$amp,$m[2]);
		...
		foreach($a as $v){
			...
			$v=explode('=',$v);
			$options.='\''.$v[0].'\'=>';
			$v[1]=str_replace($colon,':',$v[1]);
			$v[1]=str_replace($amp,'&',$v[1]);
			$v[1]=str_replace($bs,'\\',$v[1]);
			if($v[1]=='true' || $v[1]=='false'){
				$options.=$v[1].',';
			}else if(preg_match('/^[-+]*[.0-9]+$/',$v[1])){
				$options.=$v[1].',';
			}else{
				$options.='\''.urldecode($v[1]).'\',';
			}
		}
		$options='array('.rtrim($options,',').')';
		return $m[1].$options.')';
	}
}
...
function dataset_breaks_calcs($options=array()){
	...
	* 2012-04-25: we have allowed $disposition to be a function as well for both the header and the footer outputs; NOTE that there is a array $datasetColOutput
		...
		for($k=count($datasetBreakFields); $k>count($datasetBreakFields)-(count($a) - $depth); $k--){
			...
			foreach($datasetCalcFields as $field){
				if($disposition=='html-table'){
					...
					if(!function_exists($disposition))exit('Function '.$disposition.'() is not declared and group headings exited');
					$disposition(array('k'=>$k));
				}
			}
		}
	}
...
function dataset_complexDataCSS($options){
	...
	dataset_complexDataCSS(array(
		...
		'datasetColorHighlight_'=>'peru', -- highlighted
	));
	options:
		outputStyleTag: default false
		componentWrapper: allows for multiple dataobjects on a page
		useLegacyHighlighting: added 2010-11-29, old highlighting was fixed color with no mixing
	*/
	extract($options);
	if(!$datasetColorHighlight_)$datasetColorHighlight_='b0c4de';
	if($componentWrapper)$componentWrapper='.'.trim(ltrim($componentWrapper,'.')).' ';
	...
	<?php echo $componentWrapper?>.complexData tr.alt{					/* row, alt color */
		background-color:#<?php echo $datasetColorRowAlt_?>;
		}

	<?php echo $componentWrapper?>.complexData tr.alt td.sorted1{			/* row-alt, col-sorted 1 */
		background-color:#<?php echo $c1=implode('',color_mix($datasetColorRowAlt_, $datasetColorSorted_, 0.38))?>;
		}
	<?php echo $componentWrapper?>.complexData tr.alt td.sorted2{			/* row-alt, col-sorted 2 */
		background-color:#<?php echo $c2=implode('',color_mix($datasetColorRowAlt_, $datasetColorSorted_, 0.18))?>;
		}
	<?php echo $componentWrapper?>.complexData tr.alt td.sorted3{			/* row-alt, col-sorted 3 */
		background-color:#<?php echo $c3=implode('',color_mix($datasetColorRowAlt_, $datasetColorSorted_, 0.10))?>;
		}

	<?php echo $componentWrapper?>.complexData td.sorted1{				/* row-normal, col-sorted 1 */
		background-color:#<?php echo implode('',color_mix('fff', $datasetColorSorted_, 0.28))?>;
		}
	<?php echo $componentWrapper?>.complexData td.sorted2{				/* row-normal, col-sorted 2 */
		background-color:#<?php echo implode('',color_mix('fff', $datasetColorSorted_, 0.14))?>;
		}
	<?php echo $componentWrapper?>.complexData td.sorted3{				/* row-normal, col-sorted 3 */
		background-color:#<?php echo implode('',color_mix('fff', $datasetColorSorted_, 0.10))?>;
		}
	<?php if($useLegacyHighlighting){ ?>
	...
	<?php }else{ 
	/* 2010-11-29: because .alt applies to the tr, and .hlrow applies to the tr, there is no way to currently differentiate between highlighting for normal and alt rows */
	?>
	<?php echo $componentWrapper?>.complexData tr.hlrow{					/* row, highlight color */
		background-color:#<?php echo $datasetColorHighlight_?>;
		}

	<?php echo $componentWrapper?>.complexData tr.hlrow td.sorted1{			/* row-highlight, col-sorted 1 */
		background-color:#<?php echo implode('',color_mix($datasetColorHighlight_, $c1, 0.24))?>;
		}
	<?php echo $componentWrapper?>.complexData tr.hlrow td.sorted2{			/* row-highlight, col-sorted 2 */
		background-color:#<?php echo implode('',color_mix($datasetColorHighlight_, $c2, 0.12))?>;
		}
	<?php echo $componentWrapper?>.complexData tr.hlrow td.sorted3{			/* row-highlight, col-sorted 3 */
		background-color:#<?php echo implode('',color_mix($datasetColorHighlight_, $c3, 0.07))?>;
		}
	<?php } ?>
