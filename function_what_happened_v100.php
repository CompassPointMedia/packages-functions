...
function what_happened(){
	...
	if(!function_exists('q'))require('/home/phplib/public_html/devteam/php/a_f/function_q_v130.php');
	ob_start();
	print_r($GLOBALS);
	$out=ob_get_contents();
	ob_end_clean();
	if($u=$_SERVER['HTTP_USER_AGENT']){
		if($a=q("SELECT ID AS Browserstrings_ID, Bot FROM aux_browserstrings WHERE Name='".addslashes(substr($u,0,85))."'", O_ROW, $public_cnx, $public_cnx[3])){
			//ok
			extract($a);
			$isBot='IF IT IS A BOT: http://relatebase:grebeGraph1cs28@relatebase-rfm.com/admin/isbot.php?Browserstrings_ID='.$Browserstrings_ID.'&isbot=1';
			$isNotBot='IF NOT: http://relatebase:grebeGraph1cs28@relatebase-rfm.com/admin/isbot.php?Browserstrings_ID='.$Browserstrings_ID.'&isbot=0';
			if($Bot){
				//nothing
